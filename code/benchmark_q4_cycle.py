...
import q4_solver_cycle as repair
from pathlib import Path

def main():
    root=model.ROOT; out=root/'results/q4_cycle'; out.mkdir(exist_ok=True)
    ...
    lines+=['','同时充放电按已证明的等价变换消除；若仅靠循环消除仍不可行，再固定模式重新优化。仅当完整物理及合同校验通过、且重建目标不劣于已有候选时才替换。始终保留可行初解。',
    ...
    (root/'reports/Q4_CYCLE_BENCHMARK.md').write_text('\n'.join(lines)+'\n',encoding='utf8')
