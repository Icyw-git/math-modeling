...
import q4_solver_multimode as repair
from pathlib import Path

def main():
    root=model.ROOT; out=root/'results/q4_multimode'; out.mkdir(exist_ok=True)
    ...
    lines+=['','每个候选均固定完整模式并重新优化，不直接执行取整后的松弛结果。仅当完整物理及合同校验通过、且重建目标严格更低时才替换。始终保留可行初解。',
    ...
    (root/'reports/Q4_MULTIMODE_BENCHMARK.md').write_text('\n'.join(lines)+'\n',encoding='utf8')
