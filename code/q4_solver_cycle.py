...
def contract(loads,pvs,prices,w,e0,lh,vh,previous=None,charged=None,past_cash=0.,rho=.2,alpha=.9,terminal=False,rate=.4,limit=30.):
    ...
    cycle_intervals=0; cycle_charge_removed=0.; relaxation_charge_emergency=0
    ...
    for phase in ('relaxation','fixed_mode'):
        ...
        if phase=='relaxation':
            relaxed,cycle_intervals,cycle_charge_removed=eliminate_cycles(x,k,n,ns,stride,eta)
            relaxation_charge_emergency=sum(int(np.count_nonzero((relaxed[ns+j*stride:ns+j*stride+n]>1e-6)&(relaxed[ns+j*stride+3*n:ns+j*stride+4*n]>1e-6))) for j in range(k))
            x=relaxed
        checked=evaluate(x)
    ...
    return np.maximum(best[:n],0),dict(best_stats,status=selected,fallback=False,seed_used=selected=='seed',
        ...
        gap_definition='certified relaxation gap in rebuilt objective units; not a solver MIP gap',cycle_intervals=cycle_intervals,cycle_charge_removed=cycle_charge_removed,relaxation_charge_emergency=relaxation_charge_emergency)
