"""Configurable continuous-period validation of the adaptive multimode solver."""
import argparse
import os
...
import q4_solver_adaptive as adaptive
m=types.SimpleNamespace(**vars(original))
m.contract=adaptive.contract

VARIANTS={f'{mode}_{kind}':(mode,0. if kind=='neutral' else .2,kind=='known') for mode in ('q2','q3') for kind in ('neutral','cvar')}
...
def run(name,output,start_day,end_day,initial_soc):
    ...
    sources=[Path(__file__),Path(m.__file__),Path(m.en.__file__),Path(m.base.__file__),m.ROOT/'code/audit_c_attachments.py',Path(adaptive.__file__),Path(adaptive.recovery.__file__)]
    hashes={str(p.relative_to(m.ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sources}
    config=dict(variant=name,mode=mode,rho=rho,alpha=.9,known_price=known,grid_size=97,time_limit=30,scenario_days=28,source_hashes=hashes,input_hashes=data['hashes'],initial_soc=initial_soc,annual_terminal_soc=1200,period_start_day=start_day,period_end_day=end_day,version=3,solver='adaptive_multimode',recovery_limit=30.,refine_gap=.001)
    cp=out/'config.json'
    if cp.exists():
    ...
    e=float(initial_soc); begun=time.perf_counter()
    ...
    rows=[json.loads((out/f'day_{str(data["dates"][d].date())}.json').read_text(encoding='utf8')) for d in range(start_day,end_day)]
    ...
    stats.update(days=len(rows),intervals=len(rows)*144,mean_daily_fee=float(daily.total_fee.mean()),max_daily_fee=float(daily.total_fee.max()),realized_daily_cvar90=m.cvar(daily.total_fee),end_soc=e,complete=True,period_complete=True)
    dump(out/'summary.json',stats); print(name,stats,flush=True)


if __name__=='__main__':
    p=argparse.ArgumentParser(); p.add_argument('--variant',choices=list(VARIANTS),required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--start-day',type=int,required=True); p.add_argument('--end-day',type=int,required=True); p.add_argument('--initial-soc',type=float,required=True)
    a=p.parse_args(); run(a.variant,a.output,a.start_day,a.end_day,a.initial_soc)
