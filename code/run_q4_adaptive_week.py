"""Seven-day continuous validation of the opt-in adaptive solver."""
import argparse
import os
...
def run(name,output,end_day=38):
    ...
    sources=[Path(__file__),Path(m.__file__),Path(m.en.__file__),Path(m.base.__file__),m.ROOT/'code/audit_c_attachments.py',Path(adaptive.__file__),Path(repair.__file__)]
    hashes={str(p.relative_to(m.ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sources}
    config=dict(variant=name,mode=mode,rho=rho,alpha=.9,known_price=known,grid_size=97,time_limit=30,scenario_days=28,source_hashes=hashes,input_hashes=data['hashes'],initial_soc=1200,annual_terminal_soc=1200,version=2,solver="adaptive_lp_repair",recovery_limit=30.,refine_gap=.001)
    cp=out/'config.json'
    if cp.exists():
...
if __name__=='__main__':
    p=argparse.ArgumentParser(); p.add_argument('--variant',choices=list(VARIANTS),required=True); p.add_argument('--output',type=Path,default=m.ROOT/'results/q4_adaptive_week'); p.add_argument('--end-day',type=int,default=38)
