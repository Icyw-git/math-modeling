"""Independent ledger/settlement verification; never uses optimizer constraints."""
import argparse
import hashlib
import json
...
def verify(out,prefix=None):
    ...
    files=sorted(out.glob('day_*.json')); state=1200.; fee=0.; days=[]; max_error=0.; max_prefix=0.; fallback=0; windows=0; prefix_days=0
    for offset,path in enumerate(files):
        obj=json.loads(path.read_text(encoding='utf8')); day=31+offset
    ...
    assert files
    if len(files)==334: assert abs(state-1200)<1e-6
