...
This skill is installed in Codex and contains ready-to-run Python/matplotlib scripts for the figure templates exposed in the MathModel Improve tab. Resolve the skill directory from the current `SKILL.md` location.

## Fast Path

1. Match the requested chart in `references/figure-catalog.md`.
2. From the current workspace, run the renderer with the template id (`SKILL_DIR` is this skill's directory):
